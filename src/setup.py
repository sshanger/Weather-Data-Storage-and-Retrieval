...
# The compact access data information is split and stored as an array of dict objects with device id and access type in each
with open(RELATIVE_CONFIG_PATH+USER_COLLECTION+'.csv', 'r') as user_fh:
    for user_row in user_fh:
        user_row = user_row.rstrip()
        if user_row:
            (username, email, role, permissions) = user_row.split(',')
        permissions_obj = []
        if (permissions and role != 'admin'):
            permissions = permissions.split(';')
            for permission in permissions:
                (device_id, access_type) = permission.split(':')
                permission_obj = {}
                permission_obj['did'] = device_id
                permission_obj['atype'] = access_type
                permissions_obj.append(permission_obj)
        user_data = {'username': username, 'email': email, 'role': role, 'alist': permissions_obj}
